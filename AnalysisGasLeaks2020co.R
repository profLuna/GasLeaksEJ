...
unrepaired2020_with_cosub <- ma_cosub %>% 
  select(GEOID) %>%
  st_join(unrepaired2020, ., largest = TRUE)
# do same for repaired leaks
repaired2020_with_cosub <- ma_cosub %>% 
  select(GEOID) %>%
  st_join(repaired2020, ., largest = TRUE)

# break them out by leak class
unrepaired2020_with_cosubC1 <- unrepaired2020_with_cosub %>% 
  filter(Class == "1")

unrepaired2020_with_cosubC2 <- unrepaired2020_with_cosub %>% 
  filter(Class == "2")

unrepaired2020_with_cosubC3 <- unrepaired2020_with_cosub %>% 
  filter(Class == "3")
# do same for repaired
repaired2020_with_cosubC1 <- repaired2020_with_cosub %>% 
  filter(Class == "1")

repaired2020_with_cosubC2 <- repaired2020_with_cosub %>% 
  filter(Class == "2")

repaired2020_with_cosubC3 <- repaired2020_with_cosub %>% 
  filter(Class == "3")

# group by GEOID to get a simple count of all leaks within each cosub, as well as avg leak age in days
unrepaired2020_by_cosub <- unrepaired2020_with_cosub %>% 
  group_by(GEOID) %>% 
  summarize(unrepaired2020total = n(),
            LeakAgeDaysAvg = mean(LeakAgeDays, na.rm = TRUE),
            LeakAgeDaysMed = median(LeakAgeDays, na.rm = TRUE))

unrepaired2020_by_cosubC1 <- unrepaired2020_with_cosubC1 %>% 
  group_by(GEOID) %>% 
...
unrepaired2020_by_cosubC2 <- unrepaired2020_with_cosubC2 %>% 
  group_by(GEOID) %>% 
...
unrepaired2020_by_cosubC3 <- unrepaired2020_with_cosubC3 %>% 
  group_by(GEOID) %>% 
...
repaired2020_by_cosubC2 <- repaired2020_with_cosubC2 %>% 
  group_by(GEOID) %>% 
...
repaired2020_by_cosubC3 <- repaired2020_with_cosubC3 %>% 
  group_by(GEOID) %>% 
...
unrepaired2020_by_utility <- unrepaired2020_with_cosub %>% 
  as.data.frame() %>% 
...
unrepaired2020_by_utilityC1 <- unrepaired2020_with_cosubC1 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
unrepaired2020_by_utilityC2 <- unrepaired2020_with_cosubC2 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
unrepaired2020_by_utilityC3 <- unrepaired2020_with_cosubC3 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
repaired2020_by_utility <- repaired2020_with_cosub %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
repaired2020_by_utilityC1 <- repaired2020_with_cosubC1 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
repaired2020_by_utilityC2 <- repaired2020_with_cosubC2 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
repaired2020_by_utilityC3 <- repaired2020_with_cosubC3 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
ma_cosub <- lapply(list(unrepaired2020_by_cosub,
                            unrepaired2020_by_cosubC1,
                            unrepaired2020_by_cosubC2,
                            unrepaired2020_by_cosubC3), function(x){
                              as.data.frame(x) %>% 
                                select(-geometry)}) %>% 
...
# join total leak counts to the cosub demographics
ma_cosub <- lapply(list(repaired2020_by_cosub,
                            repaired2020_by_cosubC1,
                            repaired2020_by_cosubC2,
                            repaired2020_by_cosubC3), function(x){
                              as.data.frame(x) %>% 
                                select(-geometry)}) %>% 
  reduce(full_join, by = "GEOID") %>% 
  left_join(ma_cosub, ., by = "GEOID") %>% 
...
ma_cosub <- list(ma_cosub, 
  ...
  mutate(across(starts_with("AllLeaks2020"), ~ . /area_sqkm, 
                .names = "{.col}_sqkm")) %>% 
  mutate(PctRepaired19 = if_else((repaired2020total == 0 | AllLeaks2020 == 0), 
                                 0, (repaired2020total/AllLeaks2020)*100),
         PctRepaired19C1 = if_else((repaired2020totalC1 == 0 | 
                                      AllLeaks2020C1 == 0), 0,
                                   (repaired2020totalC1/AllLeaks2020C1)*100),
         PctRepaired19C2 = if_else((repaired2020totalC2 == 0 | 
                                      AllLeaks2020C2 == 0), 0,
                                   (repaired2020totalC2/AllLeaks2020C2)*100),
         PctRepaired19C3 = if_else((repaired2020totalC3 == 0 | 
                                      AllLeaks2020C3 == 0), 0,
...
saveRDS(ma_cosub, file = "Data/ma_cosub2020.Rds")
...
saveRDS(ppLeakDensityJoined, file = "Data/ppLeakDensityCO2020.Rds")

# load data
ppLeakDensityJoined <- readRDS("Data/ppLeakDensityCO2020.Rds")
...
# save object
saveRDS(ppLeakDensity_df, file = "Data/ppLeakDensity_df_cosubs2020.Rds")

# load object
ppLeakDensity_df <- readRDS("Data/ppLeakDensity_df_cosubs2020.Rds")
  ...
  mutate(leaks_sqkmNG = (`National Grid_20unrepaired`)/area_sqkm,
         ...
         AllLeaks2020_huNGC3 = if_else(total_occ_unitsE == 0, 0,
                                       (`National Grid_20unrepairedC3` + 
                                          `National Grid_20repairedC3` )/total_occ_unitsE),
         PctRepaired19NG = (`National Grid_20repaired`)/ (`National Grid_20unrepaired` + `National Grid_20repaired`)*100) %>% 
  rowwise() %>% 
  ...
  mutate(leaks_sqkmLU = (`Liberty Utilities_20unrepaired`)/area_sqkm,
         ...
         AllLeaks2020_huLUC3 = if_else(total_occ_unitsE == 0, 0,
                                       (`Liberty Utilities_20unrepairedC3` +
                                          `Liberty Utilities_20repairedC3`)/total_occ_unitsE),
         PctRepaired19LU = (`Liberty Utilities_20repaired`)/ (`Liberty Utilities_20unrepaired` + `Liberty Utilities_20repaired`)*100,
  ...
  mutate(leaks_sqkmBG = (`Berkshire Gas_20unrepaired`)/area_sqkm,
         ...
         AllLeaks2020_huBGC3 = if_else(total_occ_unitsE == 0, 0,
                                       (`Berkshire Gas_20unrepairedC3` +
                                          `Berkshire Gas_20repairedC3`)/total_occ_unitsE),
         PctRepaired19BG = (`Berkshire Gas_20repaired`)/ (`Berkshire Gas_20unrepaired` + `Berkshire Gas_20repaired`)*100,
...
# save output
saveRDS(ppLeakDensityJoinedU, file = "Data/ppLeakDensityJoinedU_CO2020.Rds")

# load data
ppLeakDensityJoinedU <- readRDS("Data/ppLeakDensityJoinedU_CO2020.Rds")
...
ggsave("Images/LeaksPPbyClassRR_cosub2020.png")
...
ggsave("Images/LeaksPPbyClassREPRR_cosub2020.png")
...
ggsave("Images/LeaksPPbyClassAllRR_cosub2020.png")
...
ggsave("Images/LeaksPPbyClass_HU_RR_cosub2020.png")
...
ggsave("Images/LeaksPPbyClassREP_HU_RR_cosub2020.png")
...
ggsave("Images/LeaksPPbyClassAll_HU_RR_cosub2020.png")
...
ggsave("Images/LeaksPPbyClassTime_RR_cosub2020.png")
...
ggsave("Images/LeaksPPbyClassAge_RR_cosub2020.png")
...
ggsave("Images/LeaksPPbyUtilityRR_cosub2020.png")
...
ggsave("Images/LeaksPPbyUtility_HU_RR_cosub2020.png")
...
ggsave("Images/LeaksPPbyUtilityREP_RR_cosub2020.png")
...
ggsave("Images/LeaksPPbyUtilityREP_HU_RR_cosub2020.png")
...
ggsave("Images/LeaksPPbyUtilityAll_RR_cosub2020.png")
...
ggsave("Images/LeaksPPbyUtilityAll_HU_RR_cosub2020.png")
...
ggsave("Images/LeaksPPbyUtilityAge_RR_cosub2020.png")
...
ggsave("Images/LeaksPPbyUtilityTime_RR_cosub2020.png")
