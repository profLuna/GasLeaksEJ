...
ng_service_areas <- st_read(dsn = "Data/townssurvey_shp",
  ...
  st_transform(., crs = st_crs(ma_cosub)) %>% 
  st_make_valid()

# assign natural gas territories to cosubs and keep only those cosubs in utility territories for which we have leaks data
ma_cosub <- ng_service_areas %>% 
  select(-TOWN) %>% 
  st_join(ma_cosub, ., largest = TRUE) %>% 
...
# assign cosub data to each unrepaired leak it intersects. 
# join cosub info to each leak that falls within it
unrepaired2019_with_cosub <- ma_cosub %>% 
  select(GEOID) %>%
  st_join(unrepaired2019final, ., largest = TRUE)
# do same for repaired leaks
repaired2019_with_cosub <- ma_cosub %>% 
  select(GEOID) %>%
  st_join(repaired2019final, ., largest = TRUE)

# break them out by leak class
unrepaired2019_with_cosubC1 <- unrepaired2019_with_cosub %>% 
  filter(Class == "1")

unrepaired2019_with_cosubC2 <- unrepaired2019_with_cosub %>% 
  filter(Class == "2")

unrepaired2019_with_cosubC3 <- unrepaired2019_with_cosub %>% 
  filter(Class == "3")
# do same for repaired
repaired2019_with_cosubC1 <- repaired2019_with_cosub %>% 
  filter(Class == "1")

repaired2019_with_cosubC2 <- repaired2019_with_cosub %>% 
  filter(Class == "2")

repaired2019_with_cosubC3 <- repaired2019_with_cosub %>% 
  filter(Class == "3")

# group by GEOID to get a simple count of all leaks within each cosub, as well as avg leak age in days
unrepaired2019_by_cosub <- unrepaired2019_with_cosub %>% 
  group_by(GEOID) %>% 
  summarize(unrepaired2019total = n(),
            LeakAgeDaysAvg = mean(LeakAgeDays, na.rm = TRUE),
            LeakAgeDaysMed = median(LeakAgeDays, na.rm = TRUE))

unrepaired2019_by_cosubC1 <- unrepaired2019_with_cosubC1 %>% 
  group_by(GEOID) %>% 
...
unrepaired2019_by_cosubC2 <- unrepaired2019_with_cosubC2 %>% 
  group_by(GEOID) %>% 
...
unrepaired2019_by_cosubC3 <- unrepaired2019_with_cosubC3 %>% 
  group_by(GEOID) %>% 
...
repaired2019_by_cosubC2 <- repaired2019_with_cosubC2 %>% 
  group_by(GEOID) %>% 
...
repaired2019_by_cosubC3 <- repaired2019_with_cosubC3 %>% 
  group_by(GEOID) %>% 
...
# group by two columns to get the count of leaks by unique combinations of Utility and GEOID so that we know which utility is responsible for which leaks in each cosub.
unrepaired2019_by_utility <- unrepaired2019_with_cosub %>% 
  as.data.frame() %>% 
...
unrepaired2019_by_utilityC1 <- unrepaired2019_with_cosubC1 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
unrepaired2019_by_utilityC2 <- unrepaired2019_with_cosubC2 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
unrepaired2019_by_utilityC3 <- unrepaired2019_with_cosubC3 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
repaired2019_by_utility <- repaired2019_with_cosub %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
repaired2019_by_utilityC1 <- repaired2019_with_cosubC1 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
repaired2019_by_utilityC2 <- repaired2019_with_cosubC2 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
repaired2019_by_utilityC3 <- repaired2019_with_cosubC3 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
# join total leak counts to the cosub demographics
ma_cosub <- lapply(list(unrepaired2019_by_cosub,
                            unrepaired2019_by_cosubC1,
                            unrepaired2019_by_cosubC2,
                            unrepaired2019_by_cosubC3), function(x){
                              as.data.frame(x) %>% 
                                select(-geometry)}) %>% 
  reduce(full_join, by = "GEOID") %>% 
  # distinct(GEOID, .keep_all = TRUE) %>% # get rid of duplicate rows
  left_join(ma_cosub, ., by = "GEOID") %>% 
...
# join leak counts by utility to the cosub demographics
ma_cosub <- list(ma_cosub, 
...
# join total leak counts to the cosub demographics
ma_cosub <- lapply(list(repaired2019_by_cosub,
                            repaired2019_by_cosubC1,
                            repaired2019_by_cosubC2,
                            repaired2019_by_cosubC3), function(x){
                              as.data.frame(x) %>% 
                                select(-geometry)}) %>% 
  reduce(full_join, by = "GEOID") %>% 
  left_join(ma_cosub, ., by = "GEOID") %>% 
...
saveRDS(ma_cosub, file = "Data/ma_cosubMEAN.Rds")

# load output
ma_cosub <- readRDS("Data/ma_cosubMEAN.Rds")

summary(ma_cosub$leaks_sqkm)
ma_cosub %>% 
  ggplot(aes(x = leaks_sqkm)) + geom_histogram()

# # Look at distributions of leak counts by cosub
# ma_cosub %>% 
#   ggplot(aes(x = unrepaired2019total)) + geom_histogram()
# 
# summary(ma_cosub$unrepaired2019total)
# 
# tm_shape(ma_cosub) + tm_fill(col = "unrepaired2019total", style = "fisher")
# 
# ma_cosub %>% 
#   ggplot(aes(x = leaks_sqkm)) + geom_histogram()
# 
# summary(ma_cosub$leaks_sqkm)
# 
# tm_shape(ma_cosub) + tm_fill(col = "leaks_sqkm", style = "quantile")
# tm_shape(ma_cosub) + tm_fill(col = "leaks_sqkm", style = "fisher") + 
...
# 
# summary(ma_cosub$unrepaired2019totalC1)
# 
# ma_cosub %>% 
#   ggplot(aes(x = unrepaired2019totalC2)) + geom_histogram()
# 
# summary(ma_cosub$unrepaired2019totalC2)
# 
# ma_cosub %>% 
#   ggplot(aes(x = unrepaired2019totalC3)) + geom_histogram()
# 
# summary(ma_cosub$unrepaired2019totalC3)
...
saveRDS(ppLeakDensityJoined, file = "Data/ppLeakDensityAVGcosubs.Rds")

# load data
ppLeakDensityJoined <- readRDS("Data/ppLeakDensityAVGcosubs.Rds")
...
# save object
saveRDS(ppLeakDensity_df, file = "Data/ppLeakDensity_df_cosubs.Rds")

# load object
ppLeakDensity_df <- readRDS("Data/ppLeakDensity_df_cosubs.Rds")
...
ggsave("Images/LeaksPPbyClass_cosub.png")
...
ggsave("Images/LeaksPPbyClassRR_cosub.png")
...
ggsave("Images/LeaksPPbyClassREP_cosub.png")
...
ggsave("Images/LeaksPPbyClassREPRR_cosub.png")
...
ggsave("Images/LeaksPPbyClassAll_cosub.png")
...
ggsave("Images/LeaksPPbyClassAllRR_cosub.png")
...
ppLeakDensity_df %>% 
  ...
  facet_wrap(~ leakClass, scales = "free") +
  labs(x = NULL, 
       y = "Population-weighted mean unrepaired leaks per occupied housing unit by Municipality",
...
ggsave("Images/LeaksPPbyClass_HU_cosub.png")
...
ppLeakDensity_df %>% 
  ...
  facet_wrap(~ leakClass, scales = "free_y") +
  labs(x = NULL, 
       y = "Ratio of group population-weighted mean leaks per occupied housing unit to population mean by Municipality",
...
ggsave("Images/LeaksPPbyClass_HU_RR_cosub.png")
...
ppLeakDensity_df %>% 
  ...
  facet_wrap(~ leakClass, scales = "free") +
  labs(x = NULL, 
       y = "Population-weighted mean repaired leaks per occupied housing unit by Municipality",
...
ggsave("Images/LeaksPPbyClassREP_HU_cosub.png")
...
ggsave("Images/LeaksPPbyClassREP_HU_RR_cosub.png")
...
ppLeakDensity_df %>% 
  ...
  facet_wrap(~ leakClass, scales = "free") +
  labs(x = NULL, 
       y = "Population-weighted mean total leaks per occupied housing unit by Municipality",
...
ggsave("Images/LeaksPPbyClassAll_HU_cosub.png")
...
ggsave("Images/LeaksPPbyClassAll_HU_RR_cosub.png")
...
ppLeakDensity_df %>% 
  ...
  facet_wrap(~ leakClass, scales = "free") +
  labs(x = NULL, 
       y = "Population-weighted mean leak repair time (days) by Municipality",
...
ggsave("Images/LeaksPPbyClassTime_cosub.png")
...
ppLeakDensity_df %>% 
  ...
  facet_wrap(~ leakClass, scales = "free_y") +
  labs(x = NULL, 
       y = "Ratio of group population-weighted mean leak repair time to population mean by Municipality",
...
ggsave("Images/LeaksPPbyClassTime_RR_cosub.png")
...
ppLeakDensity_df %>% 
  ...
  facet_wrap(~ leakClass, scales = "free") +
  labs(x = NULL, 
       y = "Population-weighted mean age (days) of unrepaired leaks by Municipality",
...
ggsave("Images/LeaksPPbyClassAge_cosub.png")
...
ppLeakDensity_df %>% 
  ...
  facet_wrap(~ leakClass, scales = "free_y") +
  labs(x = NULL, 
       y = "Ratio of group population-weighted mean leak age to population mean by Municipality",
...
ggsave("Images/LeaksPPbyUtility_cosub.png")
...
ggsave("Images/LeaksPPbyUtilityRR_cosub.png")
...
ggsave("Images/LeaksPPbyUtility_HU_cosub.png")
...
ggsave("Images/LeaksPPbyUtilityREP_cosub.png")
...
ggsave("Images/LeaksPPbyUtilityREP_RR_cosub.png")
...
ppLeakDensityJoinedU %>% 
  ...
  facet_wrap(~ Utility, scales = "free") +
  labs(x = NULL, 
       y = "Population-weighted mean of repaired leaks per occupied housing unit by Municipality",
       title = "Piority Populations and Repaired Gas Leaks Per Occupied Housing Unit by Utility for 2019")

ggsave("Images/LeaksPPbyUtilityREP_HU_cosub.png")
...
ggsave("Images/LeaksPPbyUtilityREP_HU_RR_cosub.png")
...
ggsave("Images/LeaksPPbyUtilityALL_cosub.png")
...
ggsave("Images/LeaksPPbyUtilityAll_RR_cosub.png")
...
ppLeakDensityJoinedU %>% 
  ...
  facet_wrap(~ Utility, scales = "free") +
  labs(x = NULL, 
       y = "Population-weighted mean leaks per occupied housing unity by Municipality",
       title = "Piority Populations and All Repaired and Unrepaired Gas Leaks Per Occupied Housing Unit by Utility for 2019")

ggsave("Images/LeaksPPbyUtilityALL_HU_cosub.png")
...
ggsave("Images/LeaksPPbyUtilityAll_HU_RR_cosub.png")
...
ppLeakDensityJoinedU %>% 
  ...
  facet_wrap(~ Utility, scales = "free") +
  labs(x = NULL, 
       y = "Population-weighted mean percentage of leaks repaired in 2019 by Municipality",
       title = "Piority Populations Percent Repaired Gas Leaks in 2019 by Utility across Massachusetts")

ggsave("Images/LeaksPPbyUtilityPctFix_cosub.png")
...
ggsave("Images/LeaksPPbyUtilityAge_cosub.png")
...
ggsave("Images/LeaksPPbyUtilityAge_RR_cosub.png")
...
ggsave("Images/LeaksPPbyUtilityTime_cosub.png")
...
ggsave("Images/LeaksPPbyUtilityTime_RR_cosub.png")
...
ggsave("Images/LeaksPPbyClass_NGcosub.png")
...
ggsave("Images/LeaksPPbyClassRR_NGcosub.png")
...
ggsave("Images/LeaksPPbyClassREP_NGcosub.png")
...
ggsave("Images/LeaksPPbyClassREPRR_NGcosub.png")
...
ggsave("Images/LeaksPPbyClassAll_NGcosub.png")
...
ggsave("Images/LeaksPPbyClassAllRR_NGcosub.png")
...
ggsave("Images/LeaksPPbyClass_HU_NGcosub.png")
...
ggsave("Images/LeaksPPbyClass_HU_RR_NGcosub.png")
...
ggsave("Images/LeaksPPbyClassREP_HU_NGcosub.png")
...
ggsave("Images/LeaksPPbyClassREP_HU_RR_NGcosub.png")
...
ggsave("Images/LeaksPPbyClassAll_HU_NGcosub.png")
...
ggsave("Images/LeaksPPbyClassAll_HU_RR_NGcosub.png")
...
ggsave("Images/LeaksPPbyClassTime_NGcosub.png")
...
ggsave("Images/LeaksPPbyClassTime_RR_NGcosub.png")
...
ggsave("Images/LeaksPPbyClassAge_NGcosub.png")
...
ggsave("Images/LeaksPPbyClassAge_RR_NGcosub.png")
...
ggsave("Images/LeaksPPbyClass_EVcosub.png")
...
ggsave("Images/LeaksPPbyClassRR_EVcosub.png")
...
ggsave("Images/LeaksPPbyClassREP_EVcosub.png")
...
ggsave("Images/LeaksPPbyClassREPRR_EVcosub.png")
...
ggsave("Images/LeaksPPbyClassAll_EVcosub.png")
...
ggsave("Images/LeaksPPbyClassAllRR_EVcosub.png")
...
ggsave("Images/LeaksPPbyClass_HU_EVcosub.png")
...
ggsave("Images/LeaksPPbyClass_HU_RR_EVcosub.png")
...
ggsave("Images/LeaksPPbyClassREP_HU_EVcosub.png")
...
ggsave("Images/LeaksPPbyClassREP_HU_RR_EVcosub.png")
...
ggsave("Images/LeaksPPbyClassAll_HU_EVcosub.png")
...
ggsave("Images/LeaksPPbyClassAll_HU_RR_EVcosub.png")
...
ggsave("Images/LeaksPPbyClassTime_EVcosub.png")
...
ggsave("Images/LeaksPPbyClassTime_RR_EVcosub.png")
...
ggsave("Images/LeaksPPbyClassAge_EVcosub.png")
...
ggsave("Images/LeaksPPbyClassAge_RR_EVcosub.png")
...
ggsave("Images/LeaksPPbyClass_CGcosub.png")
...
ggsave("Images/LeaksPPbyClassRR_CGcosub.png")
...
ggsave("Images/LeaksPPbyClassREP_CGcosub.png")
...
ggsave("Images/LeaksPPbyClassREPRR_CGcosub.png")
...
ggsave("Images/LeaksPPbyClassAll_CGcosub.png")
...
ggsave("Images/LeaksPPbyClassAllRR_CGcosub.png")
...
ggsave("Images/LeaksPPbyClass_HU_CGcosub.png")
...
ggsave("Images/LeaksPPbyClass_HU_RR_CGcosub.png")
...
ggsave("Images/LeaksPPbyClassREP_HU_CGcosub.png")
...
ggsave("Images/LeaksPPbyClassREP_HU_RR_CGcosub.png")
...
ggsave("Images/LeaksPPbyClassAll_HU_CGcosub.png")
...
ggsave("Images/LeaksPPbyClassAll_HU_RR_CGcosub.png")
...
ggsave("Images/LeaksPPbyClassTime_CGcosub.png")
...
ggsave("Images/LeaksPPbyClassTime_RR_CGcosub.png")
...
ggsave("Images/LeaksPPbyClassAge_CGcosub.png")
...
ggsave("Images/LeaksPPbyClassAge_RR_CGcosub.png")
...
ggsave("Images/LeaksPPbyClass_LUcosub.png")
...
ggsave("Images/LeaksPPbyClassRR_LUcosub.png")
...
ggsave("Images/LeaksPPbyClassREP_LUcosub.png")
...
ggsave("Images/LeaksPPbyClassREPRR_LUcosub.png")
...
ggsave("Images/LeaksPPbyClassAll_LUcosub.png")
...
ggsave("Images/LeaksPPbyClassAllRR_LUcosub.png")
...
ggsave("Images/LeaksPPbyClass_HU_LUcosub.png")
...
ggsave("Images/LeaksPPbyClass_HU_RR_LUcosub.png")
...
ggsave("Images/LeaksPPbyClassREP_HU_LUcosub.png")
...
ggsave("Images/LeaksPPbyClassREP_HU_RR_LUcosub.png")
...
ggsave("Images/LeaksPPbyClassAll_HU_LUcosub.png")
...
ggsave("Images/LeaksPPbyClassAll_HU_RR_LUcosub.png")
...
ggsave("Images/LeaksPPbyClassTime_LUcosub.png")
...
ggsave("Images/LeaksPPbyClassTime_RR_LUcosub.png")
...
ggsave("Images/LeaksPPbyClassAge_LUcosub.png")
...
ggsave("Images/LeaksPPbyClassAge_RR_LUcosub.png")
...
ggsave("Images/LeaksPPbyClass_BGcosub.png")
...
ggsave("Images/LeaksPPbyClassRR_BGcosub.png")
...
ggsave("Images/LeaksPPbyClassREP_BGcosub.png")
...
ggsave("Images/LeaksPPbyClassREPRR_BGcosub.png")
...
ggsave("Images/LeaksPPbyClassAll_BGcosub.png")
...
ggsave("Images/LeaksPPbyClassAllRR_BGcosub.png")
...
ggsave("Images/LeaksPPbyClass_HU_BGcosub.png")
...
ggsave("Images/LeaksPPbyClass_HU_RR_BGcosub.png")
...
ggsave("Images/LeaksPPbyClassREP_HU_BGcosub.png")
...
ggsave("Images/LeaksPPbyClassREP_HU_RR_BGcosub.png")
...
ggsave("Images/LeaksPPbyClassAll_HU_BGcosub.png")
...
ggsave("Images/LeaksPPbyClassAll_HU_RR_BGcosub.png")
...
ggsave("Images/LeaksPPbyClassTime_BGcosub.png")
...
ggsave("Images/LeaksPPbyClassTime_RR_BGcosub.png")
...
ggsave("Images/LeaksPPbyClassAge_BGcosub.png")
...
ggsave("Images/LeaksPPbyClassAge_RR_BGcosub.png")
...
(pwdt_leaks_sqkm %>% 
    ...
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_leaks_sqkm.csv"))
...
(pwdt_leaks_sqkmC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_leaks_sqkmC1.csv"))
...
(pwdt_leaks_sqkmC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_leaks_sqkmC2.csv"))
...
(pwdt_leaks_sqkmC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_leaks_sqkmC3.csv"))
...
(pwdt_REPleaks_sqkm %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_REPleaks_sqkm.csv"))
...
(pwdt_REPleaks_sqkmC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_REPleaks_sqkmC1.csv"))
...
(pwdt_REPleaks_sqkmC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_REPleaks_sqkmC2.csv"))
...
(pwdt_REPleaks_sqkmC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_REPleaks_sqkmC3.csv"))
...
(pwdt_leaks_hu %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_leaks_hu.csv"))
...
(pwdt_leaks_huC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_leaks_huC1.csv"))
...
(pwdt_leaks_huC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_leaks_huC2.csv"))
...
(pwdt_leaks_huC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_leaks_huC3.csv"))
...
(pwdt_REPleaks_hu %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_REPleaks_hu.csv"))
...
(pwdt_REPleaks_huC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_REPleaks_huC1.csv"))
...
(pwdt_REPleaks_huC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_REPleaks_huC2.csv"))
...
(pwdt_REPleaks_huC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_REPleaks_huC3.csv"))
...
(pwdt_DaysToRepairAvg %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_DaysToRepairAvg.csv"))
...
(pwdt_DaysToRepairAvgC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_DaysToRepairAvgC1.csv"))
...
(pwdt_DaysToRepairAvgC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_DaysToRepairAvgC2.csv"))
...
(pwdt_DaysToRepairAvgC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_DaysToRepairAvgC3.csv"))
...
(pwdt_LeakAgeDaysAvg %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_LeakAgeDaysAvg.csv"))
...
(pwdt_LeakAgeDaysAvgC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_LeakAgeDaysAvgC1.csv"))
...
(pwdt_LeakAgeDaysAvgC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_LeakAgeDaysAvgC2.csv"))
...
(pwdt_LeakAgeDaysAvgC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/pwdt_LeakAgeDaysAvgC3.csv"))
...
(pwdt_leaks_sqkmNG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_leaks_sqkmNG.csv"))
...
(pwdt_leaks_sqkmNGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_leaks_sqkmNGC1.csv"))
...
(pwdt_leaks_sqkmNGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_leaks_sqkmNGC2.csv"))
...
(pwdt_leaks_sqkmNGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_leaks_sqkmNGC3.csv"))
...
(pwdt_REPleaks_sqkmNG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_REPleaks_sqkmNG.csv"))
...
(pwdt_REPleaks_sqkmNGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_REPleaks_sqkmNGC1.csv"))
...
(pwdt_REPleaks_sqkmNGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_REPleaks_sqkmNGC2.csv"))
...
(pwdt_REPleaks_sqkmNGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_REPleaks_sqkmNGC3.csv"))
...
(pwdt_leaks_huNG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_leaks_huNG.csv"))
...
(pwdt_leaks_huNGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_leaks_huNGC1.csv"))
...
(pwdt_leaks_huNGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_leaks_huNGC2.csv"))
...
(pwdt_leaks_huNGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_leaks_huNGC3.csv"))
...
(pwdt_REPleaks_huNG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_REPleaks_huNG.csv"))
...
(pwdt_REPleaks_huNGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_REPleaks_huNGC1.csv"))
...
(pwdt_REPleaks_huNGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_REPleaks_huNGC2.csv"))
...
(pwdt_REPleaks_huNGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_REPleaks_huNGC3.csv"))
...
(pwdt_DaysToRepairAvgNG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_DaysToRepairAvgNG.csv"))
...
(pwdt_DaysToRepairAvgNGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_DaysToRepairAvgNGC1.csv"))
...
(pwdt_DaysToRepairAvgNGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_DaysToRepairAvgNGC2.csv"))
...
(pwdt_DaysToRepairAvgNGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_DaysToRepairAvgNGC3.csv"))
...
(pwdt_LeakAgeDaysAvgNG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_LeakAgeDaysAvgNG.csv"))
...
(pwdt_LeakAgeDaysAvgNGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_LeakAgeDaysAvgNGC1.csv"))
...
(pwdt_LeakAgeDaysAvgNGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_LeakAgeDaysAvgNGC2.csv"))
...
(pwdt_LeakAgeDaysAvgNGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/NG/pwdt_LeakAgeDaysAvgNGC3.csv"))
...
(pwdt_leaks_sqkmEV %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_leaks_sqkmEV.csv"))
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/EV/pwdt_leaks_sqkmEVC1.csv"))
...
(pwdt_leaks_sqkmEVC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_leaks_sqkmEVC2.csv"))
...
(pwdt_leaks_sqkmEVC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_leaks_sqkmEVC3.csv"))
...
(pwdt_REPleaks_sqkmEV %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_REPleaks_sqkmEV.csv"))
...
(pwdt_REPleaks_sqkmEVC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_REPleaks_sqkmEVC1.csv"))
...
(pwdt_REPleaks_sqkmEVC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_REPleaks_sqkmEVC2.csv"))
...
(pwdt_REPleaks_sqkmEVC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_REPleaks_sqkmEVC3.csv"))
...
(pwdt_leaks_huEV %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_leaks_huEV.csv"))
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/EV/pwdt_leaks_huEVC1.csv"))
...
(pwdt_leaks_huEVC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_leaks_huEVC2.csv"))
...
(pwdt_leaks_huEVC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_leaks_huEVC3.csv"))
...
(pwdt_REPleaks_huEV %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_REPleaks_huEV.csv"))
...
(pwdt_REPleaks_huEVC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_REPleaks_huEVC1.csv"))
...
(pwdt_REPleaks_huEVC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_REPleaks_huEVC2.csv"))
...
(pwdt_REPleaks_huEVC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_REPleaks_huEVC3.csv"))
...
(pwdt_DaysToRepairAvgEV %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_DaysToRepairAvgEV.csv"))
...
(pwdt_DaysToRepairAvgEVC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_DaysToRepairAvgEVC1.csv"))
...
(pwdt_DaysToRepairAvgEVC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_DaysToRepairAvgEVC2.csv"))
...
(pwdt_DaysToRepairAvgEVC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_DaysToRepairAvgEVC3.csv"))
...
(pwdt_LeakAgeDaysAvgEV %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_LeakAgeDaysAvgEV.csv"))
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/EV/pwdt_LeakAgeDaysAvgEVC1.csv"))
...
(pwdt_LeakAgeDaysAvgEVC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_LeakAgeDaysAvgEVC2.csv"))
...
(pwdt_LeakAgeDaysAvgEVC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/EV/pwdt_LeakAgeDaysAvgEVC3.csv"))
...
(pwdt_leaks_sqkmCG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_leaks_sqkmCG.csv"))
...
(pwdt_leaks_sqkmCGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_leaks_sqkmCGC1.csv"))
...
(pwdt_leaks_sqkmCGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_leaks_sqkmCGC2.csv"))
...
(pwdt_leaks_sqkmCGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_leaks_sqkmCGC3.csv"))
...
(pwdt_REPleaks_sqkmCG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_REPleaks_sqkmCG.csv"))
...
(pwdt_REPleaks_sqkmCGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_REPleaks_sqkmCGC1.csv"))
...
(pwdt_REPleaks_sqkmCGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_REPleaks_sqkmCGC2.csv"))
...
(pwdt_REPleaks_sqkmCGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_REPleaks_sqkmCGC3.csv"))
...
(pwdt_leaks_huCG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_leaks_huCG.csv"))
...
(pwdt_leaks_huCGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_leaks_huCGC1.csv"))
...
(pwdt_leaks_huCGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_leaks_huCGC2.csv"))
...
(pwdt_leaks_huCGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_leaks_huCGC3.csv"))
...
(pwdt_REPleaks_huCG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_REPleaks_huCG.csv"))
...
(pwdt_REPleaks_huCGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_REPleaks_huCGC1.csv"))
...
(pwdt_REPleaks_huCGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_REPleaks_huCGC2.csv"))
...
(pwdt_REPleaks_huCGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_REPleaks_huCGC3.csv"))
...
(pwdt_DaysToRepairAvgCG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_DaysToRepairAvgCG.csv"))
...
(pwdt_DaysToRepairAvgCGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_DaysToRepairAvgCGC1.csv"))
...
(pwdt_DaysToRepairAvgCGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_DaysToRepairAvgCGC2.csv"))
...
(pwdt_DaysToRepairAvgCGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_DaysToRepairAvgCGC3.csv"))
...
(pwdt_LeakAgeDaysAvgCG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_LeakAgeDaysAvgCG.csv"))
...
(pwdt_LeakAgeDaysAvgCGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_LeakAgeDaysAvgCGC1.csv"))
...
(pwdt_LeakAgeDaysAvgCGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_LeakAgeDaysAvgCGC2.csv"))
...
(pwdt_LeakAgeDaysAvgCGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/CG/pwdt_LeakAgeDaysAvgCGC3.csv"))
...
(pwdt_leaks_sqkmFG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/FG/pwdt_leaks_sqkmFG.csv"))
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_leaks_sqkmFGC1.csv"))
# 
# # C2 unrepaired leaks per sqkm
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_leaks_sqkmFGC2.csv"))
# 
# # C3 unrepaired leaks per sqkm
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_leaks_sqkmFGC3.csv"))
...
(pwdt_REPleaks_sqkmFG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/FG/pwdt_REPleaks_sqkmFG.csv"))
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_REPleaks_sqkmFGC1.csv"))
# 
# # C2 repaired leaks per sqkm
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_REPleaks_sqkmFGC2.csv"))
# 
# # C3 repaired leaks per sqkm
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_REPleaks_sqkmFGC3.csv"))
...
(pwdt_leaks_huFG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/FG/pwdt_leaks_huFG.csv"))
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_leaks_huFGC1.csv"))
# 
# # C2 unrepaired leaks per OHU
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_leaks_huFGC2.csv"))
# 
# # C3 unrepaired leaks per OHU
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_leaks_huFGC3.csv"))
...
(pwdt_REPleaks_huFG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/FG/pwdt_REPleaks_huFG.csv"))
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_REPleaks_huFGC1.csv"))
# 
# # C2 repaired leaks per OHU
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_REPleaks_huFGC2.csv"))
# 
# # C3 repaired leaks per OHU
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_REPleaks_huFGC3.csv"))
...
(pwdt_DaysToRepairAvgFG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/FG/pwdt_DaysToRepairAvgFG.csv"))
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_DaysToRepairAvgFGC1.csv"))
# 
# # C2 days to repair avg
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_DaysToRepairAvgFGC2.csv"))
# 
# # C3 days to repair avg
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_DaysToRepairAvgFGC3.csv"))
...
(pwdt_LeakAgeDaysAvgFG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/FG/pwdt_LeakAgeDaysAvgFG.csv"))
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_LeakAgeDaysAvgFGC1.csv"))
# 
# # C2 leak age days avg
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_LeakAgeDaysAvgFGC2.csv"))
# 
# # C3 leak age days avg
...
#              (group2 == "under5E" & group1 == "totalpopE")) %>% 
#     arrange(statistic) %>% 
#     write_csv("Tables/CoSub/FG/pwdt_LeakAgeDaysAvgFGC3.csv"))
...
(pwdt_leaks_sqkmLU %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_leaks_sqkmLU.csv"))
...
(pwdt_leaks_sqkmLUC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_leaks_sqkmLUC1.csv"))
...
(pwdt_leaks_sqkmLUC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_leaks_sqkmLUC2.csv"))
...
(pwdt_leaks_sqkmLUC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_leaks_sqkmLUC3.csv"))
...
(pwdt_REPleaks_sqkmLU %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_REPleaks_sqkmLU.csv"))
...
(pwdt_REPleaks_sqkmLUC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_REPleaks_sqkmLUC1.csv"))
...
(pwdt_REPleaks_sqkmLUC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_REPleaks_sqkmLUC2.csv"))
...
(pwdt_REPleaks_sqkmLUC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_REPleaks_sqkmLUC3.csv"))
...
(pwdt_leaks_huLU %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_leaks_huLU.csv"))
...
(pwdt_leaks_huLUC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_leaks_huLUC1.csv"))
...
(pwdt_leaks_huLUC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_leaks_huLUC2.csv"))
...
(pwdt_leaks_huLUC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_leaks_huLUC3.csv"))
...
(pwdt_REPleaks_huLU %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_REPleaks_huLU.csv"))
...
(pwdt_REPleaks_huLUC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_REPleaks_huLUC1.csv"))
...
(pwdt_REPleaks_huLUC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_REPleaks_huLUC2.csv"))
...
(pwdt_REPleaks_huLUC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_REPleaks_huLUC3.csv"))
...
(pwdt_DaysToRepairAvgLU %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_DaysToRepairAvgLU.csv"))
...
(pwdt_DaysToRepairAvgLUC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_DaysToRepairAvgLUC1.csv"))
...
(pwdt_DaysToRepairAvgLUC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_DaysToRepairAvgLUC2.csv"))
...
(pwdt_DaysToRepairAvgLUC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_DaysToRepairAvgLUC3.csv"))
...
(pwdt_LeakAgeDaysAvgLU %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_LeakAgeDaysAvgLU.csv"))
...
(pwdt_LeakAgeDaysAvgLUC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_LeakAgeDaysAvgLUC1.csv"))
...
(pwdt_LeakAgeDaysAvgLUC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_LeakAgeDaysAvgLUC2.csv"))
...
(pwdt_LeakAgeDaysAvgLUC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/LU/pwdt_LeakAgeDaysAvgLUC3.csv"))
...
(pwdt_leaks_sqkmBG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_leaks_sqkmBG.csv"))
...
(pwdt_leaks_sqkmBGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_leaks_sqkmBGC1.csv"))
...
(pwdt_leaks_sqkmBGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_leaks_sqkmBGC2.csv"))
...
(pwdt_leaks_sqkmBGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_leaks_sqkmBGC3.csv"))
...
(pwdt_REPleaks_sqkmBG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_REPleaks_sqkmBG.csv"))
...
(pwdt_REPleaks_sqkmBGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_REPleaks_sqkmBGC1.csv"))
...
(pwdt_REPleaks_sqkmBGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_REPleaks_sqkmBGC2.csv"))
...
(pwdt_REPleaks_sqkmBGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_REPleaks_sqkmBGC3.csv"))
...
(pwdt_leaks_huBG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>%  
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_leaks_huBG.csv"))
...
(pwdt_leaks_huBGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_leaks_huBGC1.csv"))
...
(pwdt_leaks_huBGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_leaks_huBGC2.csv"))
...
(pwdt_leaks_huBGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_leaks_huBGC3.csv"))
...
(pwdt_REPleaks_huBG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_REPleaks_huBG.csv"))
...
(pwdt_REPleaks_huBGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_REPleaks_huBGC1.csv"))
...
(pwdt_REPleaks_huBGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_REPleaks_huBGC2.csv"))
...
(pwdt_REPleaks_huBGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_REPleaks_huBGC3.csv"))
...
(pwdt_DaysToRepairAvgBG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_DaysToRepairAvgBG.csv"))
...
(pwdt_DaysToRepairAvgBGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_DaysToRepairAvgBGC1.csv"))
...
(pwdt_DaysToRepairAvgBGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_DaysToRepairAvgBGC2.csv"))
...
(pwdt_DaysToRepairAvgBGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_DaysToRepairAvgBGC3.csv"))
...
(pwdt_LeakAgeDaysAvgBG %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_LeakAgeDaysAvgBG.csv"))
...
(pwdt_LeakAgeDaysAvgBGC1 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_LeakAgeDaysAvgBGC1.csv"))
...
(pwdt_LeakAgeDaysAvgBGC2 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_LeakAgeDaysAvgBGC2.csv"))
...
(pwdt_LeakAgeDaysAvgBGC3 %>% 
    ...
    filter(group2 == "totalpopE" | 
             ...
             (group2 == "under5E" & group1 == "totalpopE")) %>% 
    arrange(statistic) %>% 
    write_csv("Tables/CoSub/BG/pwdt_LeakAgeDaysAvgBGC3.csv"))



######## Look at unrepaired stats per cosub
...
ma_cosub %>% 
  ...
  theme_minimal() +
  labs(title = "Unrepaired Leaks Per Municipality in 2019", x = "leak class")

ggsave("Data/boxplotCoSubunrepaired.png")

# what pct of cosubs with unrepaired leaks? Almost all (91%) of cosubs in gas service territories had at least 1 unrepaired gas leak in 2019
ma_cosub %>% 
  filter(unrepaired2019total > 0) %>% 
  nrow()/nrow(ma_cosub)

# Look at repaired stats per cosub
ma_cosub %>% 
  as.data.frame() %>% 
...
ma_cosub %>% 
  ...
  ggplot(aes(x = Class, y = count)) + geom_boxplot() +
  theme_minimal() +
  labs(title = "Repaired Leaks Per Municipality in 2019", x = "leak class")

ggsave("Data/boxplotCoSubrepaired.png")

# what pct of cosubs with repaired leaks? Almost all (93%) of cosubs in gas service territories had at least 1 repaired gas leak in 2019
ma_cosub %>% 
  filter(repaired2019total > 0) %>% 
  nrow()/nrow(ma_cosub)

# look at all leaks per cosub
ma_cosub %>% 
  as.data.frame() %>% 
...
ma_cosub %>% 
  ...
  ggplot(aes(x = Class, y = count)) + geom_boxplot() +
  theme_minimal() +
  labs(title = "Total Leaks Per Municipality in 2019", x = "leak class")

ggsave("Data/boxplotCoSuballleaks.png")

# what pct of cosubs with any reported leaks? Almost all (95.7%) of cosubs in gas service territories had at least 1 reported gas leak in 2019
ma_cosub %>% 
...
gridCnt <- st_make_grid(x = ma_cosub, cellsize = 1000, square = FALSE) %>% 
  st_as_sf() %>% 
...
gridCnt1 <- gridCnt %>% 
  ...
  crop_shape(., ma_cosub, polygon = TRUE) %>% 
  st_make_valid()
...
# map out leak densities by cosub
ma_cosub %>% 
...
ma_cosub_sp <- as_Spatial(ma_cosub)

# generate empirical semivariogram
v <- variogram(AllLeaks2019_sqkm ~ 1, ma_cosub_sp, cutoff = 20000, width = 1000)
v
...
nb <- dnearneigh(coordinates(ma_cosub_sp),0,15000)
...
local_g <- localG(ma_cosub$leaks_sqkm, listw)
local_g <- cbind(ma_cosub, as.matrix(local_g))
...
neighborsQC <- poly2nb(ma_cosub, queen = TRUE)
...
# 
# ggsave("Images/LeaksPP_cosub_moe.png")
# 
...
# 
# ggsave("Images/LeaksPP_HU_cosub_moe.png")
# 
...
# 
# ggsave("Images/LeaksPPall_cosub_moe.png")
# 
...
# 
# ggsave("Images/LeaksPPall_HU_cosub_moe.png")
# 
...
# 
# ggsave("Images/LeaksPPrepaired_cosub_moe.png")
# 
...
#   ggtitle("Priority Populations and Repaired Gas Leaks\nPer Housing Unit in 2019 across Massachusetts")
# 
# ggsave("Images/LeaksPPrepaired_HU_cosub_moe.png")
# 
...
# 
# ggsave("Images/LeaksPPage_cosub_moe.png")
# 
# 
...
#   geom_col() + coord_flip() + xlab("") + 
#   ylab("Population-weighted mean leak repair time (days) by Municipality") +
#   theme_minimal() +
...
# 
# ggsave("Images/LeaksPPtimeRep_cosub_moe.png")
# 
...
# 
# ggsave("Images/LeaksPPpctFixed_cosub_moe.png")
# 
# 
...
#   facet_wrap(~ leakClass, scales = "free") +
#   labs(x = NULL, 
#        y = "Population-weighted mean leaks per occupied housing unit by Municipality",
...
# 
# ggsave("Images/LeaksPPbyClassPctFix_cosub.png")
# 
# 
# 
# 
...
#   facet_wrap(~ Utility, scales = "free") +
#   labs(x = NULL, 
#        y = "Population-weighted mean percentage of leak repair time(days) by Municipality",
#        title = "Piority Populations and Average Leak Repair Time in 2019 by Utility across Massachusetts")
# 
# ggsave("Images/LeaksPPbyUtilityTime_cosub.png")
# 
...
# # library(weights)
# weights::wtd.t.test(x = ma_cosub$leaks_sqkm, y = ma_cosub$leaks_sqkm, 
#                     weight = ma_cosub$nhwhitepop_E, 
#                     weighty = ma_cosub$nhblackpop_E, 
#                     bootse = TRUE, bootp = TRUE)
# 
# weights::wtd.t.test(x = ma_cosub$PctRepaired19, 
#                     y = ma_cosub$PctRepaired19, 
#                     weight = ma_cosub$nhwhitepop_E, 
#                     weighty = ma_cosub$eng_limitE, 
#                     bootse = TRUE, bootp = TRUE)
