...
Gas leaks data for this analysis were acquired from HEET (https://heetma.org/gas-leaks/gas-leak-maps/). HEET’s data on gas leaks is derived from Annual Service Quality Reports (ASQR) that are submitted by investor-owned utilities to the Massachusetts Department of Public Utilities (DPU) each February, which reflect gas leak activity for the previous calendar year [@HEET_2021; @GasLeakAct]. HEET geocoded the leaks based on their reported addresses or cross-street locations and performed extensive quality control on geocoded results. HEET’s gas leak data contain all of the original data reported in the AQSRs, including addresses and geocoded coordinate locations, date first reported, and leak class or grade (i.e., explosive hazard) of individual repaired and unrepaired gas leaks for each utility for one year – January 1 through December 31, 2019.
...
As of 2018, gas utilities are also required to repair environmentally significant grade 3 leaks (SEIs) on an accelerated schedule, within one to five years depending on the leak extent, but SEIs are not evaluated here due to the variability in classification accuracy. 

**INSERT EXPLANATION OF ‘ELIMNATED LEAKS’ TO DISTINGUISH FROM ‘REPAIRED’ LEAKS AND WHY WE DID NOT INCORPORATE THEM**

Although 2020 gas leaks data were available at the time of this analysis, the results of that analysis are not presented here due to concerns about their representativeness given the impact of the pandemic. Results of the 2020 analysis are largely consistent with the 2019 analysis presented here. Those results are accessible online in supplementary materials. MAYBE INCLUDE THIS AS A FOOTNOTE.

Population data was derived from the American Community Survey 5-year estimates for 2015-2019 at the Census Block Group level, Census Tract level, and the municipality level (i.e., Census County Subdivisions) via the tidycensus package in R (Walker and Herman 2021).   
...
A population-weighted mean is calculated according the following formula:
...
$$ RE = \frac{\overline{x_w} subgroup} {\overline{x_w} total pop} $$
where $RE$ is the relative exposure expressed as a ratio;
where $\overline{x_w} subgroup$ is the population-weighted mean exposure of a subgroup, such as People of Color;
where $\overline{x_w} total pop$ is the population-weighted mean exposure of the total or comparison population.
...
* In addition to the aggregate category of “People of Color”, non-Hispanic Asians, non-Hispanic Blacks, and Hispanics, and non-Hispanic Whites were analyzed separately as well
...
```{r MAUP, fig.align = "center", fig.cap="Comparison of scales and units of analysis: municiplaity (i.e., county subdivision), Census Tract, and Census Block Group"}
...
a <- tm_shape(a_bm) + tm_rgb() + 
  ...
  tm_layout(title = "Municipality\n(i.e., County Subdivision)", 
...
newton <- county_subdivisions(state = "MA", cb = TRUE) %>% 
  ...
  st_centroid(of_largest_polygon = TRUE)
```
In calendar year 2019, there were `r format(nrow(unrepaired2019)+nrow(repaired2019), big.mark = ",")` natural gas leaks in the distribution system reported by seven of the eight investor-owned utilities in Massachusetts that filed leak reports with the state Department of Public Utilities. Repairs were reported for `r format(nrow(repaired2019), big.mark = ",")` (`r round(nrow(repaired2019)/(nrow(unrepaired2019)+nrow(repaired2019)),2)`%) of these, leaving `r format(nrow(unrepaired2019), big.mark = ",")` (`r round(nrow(unrepaired2019)/(nrow(unrepaired2019)+nrow(repaired2019)),2)`%) unrepaired leaks across the state as of December 31, 2019 (see \@ref(tab:tabLeaks)). 
...
```
```{r mapAllLeaksCO, fig.align='center', fig.cap= "Massachusetts gas leaks per square kilometer by municipality (i.e., Census County Subdivision), 2019",cache=TRUE}
...
ppLeakDensity_df_all2 %>% 
  ...
  labs(x = NULL, 
       ...
       title = "Relative Exposure to Unrepaired Gas Leaks in 2019 across Massachusetts", caption = "Shapes indicate scale or unit of analysis. Colors indicate if exposure is above (orange), below (blue), or equal to (gray) general population. Colors of horizontal bars through shapes indicate if exposure is consistently\nabove (orange) or below (blue), or if it is mixed (gray). Environmental Justice communities ('MA') only evaluated at Block Group scale. Disabled Adults and Housing Burdened only evaluated at Tract and Municipality scales.")
...
ppLeakDensityJoinedU_all2 %>% 
  ...
  labs(x = NULL, 
       ...
       title = "Relative Exposure to Unrepaired Gas Leaks in 2019 across Massachusetts by Utility", caption = "Shapes indicate scale or unit of analysis. Colors indicate if exposure is above (orange), below (blue), or equal to (gray) general population. Colors of horizontal bars through shapes indicate if exposure is consistently\nabove (orange) or below (blue), or if it is mixed (gray). Environmental Justice communities ('MA') only evaluated at Block Group scale. Disabled Adults and Housing Burdened only evaluated at Tract and Municipality scales.")
...
```{r mapOHUco, fig.align='center', fig.cap= "Massachusetts gas leaks per occupied housing unit by municipality (i.e., Census County Subdivision), 2019", cache=TRUE}
...
ppLeakDensityJoinedU_bg2 <- ppLeakDensityJoinedU_bg %>% 
  ...
  mutate(Group = recode(Group, "MA_ENGLISH" = "MA Limited English HH",
                        "MA_MINORITY21" = "MA Minority",
                        "MA_INCOME21" = "MA Low Income"),
         Utility = recode(Utility, "wLeaksPerHURRBGC2" = "Berkshire Gas",
                          "wLeaksPerHURRCGC2" = "Columbia Gas",
                          "wLeaksPerHURREVC2" = "Eversource Energy",
                          "wLeaksPerHURRLUC2" = "Liberty Utilities",
                          "wLeaksPerHURRNGC2" = "National Grid",
                          "wLeaksPerHURRFGC2" = "Unitil/Fitchburg Gas"),
         # Group = reorder_within(Group, leakDensityBG, Utility),
         Status = case_when(
           leakDensityBG == 1 ~ "Pop Mean",
           leakDensityBG > 1 ~ "Greater Exposure",
...
ppLeakDensityJoinedU_tr2 <- ppLeakDensityJoinedU_tr %>% 
  ...
  filter(!Group %in% c("Native American", "Other race", 
                       "Native Pacific Islander", "Two or more races")) %>% 
  drop_na(leakDensityTR) %>%
  mutate(Utility = recode(Utility, "wLeaksPerHURRBGC2" = "Berkshire Gas",
                          "wLeaksPerHURRCGC2" = "Columbia Gas",
                          "wLeaksPerHURREVC2" = "Eversource Energy",
                          "wLeaksPerHURRLUC2" = "Liberty Utilities",
                          "wLeaksPerHURRNGC2" = "National Grid",
...
ppLeakDensityJoinedU_all2 %>% 
  ...
  labs(x = NULL, 
       y = "Ratio of group population-weighted mean leaks per occupied housing unit to population mean",
       title = "Relative Exposure to Unrepaired Class 2 Leaks Per Occupied Housing Unit in 2019 across Massachusetts by Utility", caption = "Shapes indicate scale or unit of analysis. Colors indicate if exposure is above (orange), below (blue), or equal to (gray) general population. Colors of horizontal bars through shapes indicate if exposure is consistently\nabove (orange) or below (blue), or if it is mixed (gray). Environmental Justice communities ('MA') only evaluated at Block Group scale. Disabled Adults and Housing Burdened only evaluated at Tract and Municipality scales.")
...
```{r histREP, fig.align='center', fig.cap="Histogram of leaks repaired in 2019 by leak class"}
...
There is no clear geographic pattern or clustering of areas with the longer repair times (see Figures \@ref(fig:mapTimeBG) - \@ref(fig:mapTimeCO)). Indeed, communities with the longest repair times are scattered throughout the state. Notably, these longest repair times are not concentrated within major urban areas but are instead in outlying suburbs.

```{r mapTimeBG, fig.align='center', fig.cap= "Average repair times for leaks repaired in 2019 across Massachuseetts by Census Block Group", cache=TRUE}
...
```{r mapTimeTR, fig.align='center', fig.cap= "Average repair times for leaks repaired in 2019 across Massachuseetts by Census Tract", cache=TRUE}
...
```{r mapTimeCO, fig.align='center', fig.cap= "Average repair times for leaks repaired in 2019 across Massachuseetts by municipality (i.e., Census County Subdivision)", cache=TRUE}
...
For all leak classes combined, People of Color (especially Blacks and Asians) and renters experience the longest repair times; higher relative exposures to population-weighted mean leak repair times compared to the general population (see Figure \@ref(fig:figDotTime)). This pattern holds true for Class 2 and Class 3 leaks. For Class 1 leaks, residents of low income environmental justice communities have the greatest relative exposure. Whites also experience consistently greater population-weighted repair times for Class 1 leaks.  Note however that the differences in repair time between Class 1 leaks average only a few days, while differences in Class 2 and 3 leaks average repair times are more than a year. 

```{r figDotTime, fig.align='center', fig.cap="Relative exposures to population-weighted mean gas leak repair times in 2019 at Census Block Group, Census Tract, and municipality scales"}
# dot graph of relative exposure for unrepaired leak density around zero line
cols <- c("#F7A35C", "#7CB5EC", "gray30")
shps <- c(0,2,1)
...
ppLeakDensity_df_all2 %>% 
  ...
  labs(x = NULL, 
       ...
       title = "Relative Exposure to Mean Leak Repair Time in 2019 across Massachusetts", caption = "Shapes indicate scale or unit of analysis. Colors indicate if exposure is above (orange), below (blue), or equal to (gray) general population. Colors of horizontal bars through shapes indicate if exposure is consistently\nabove (orange) or below (blue), or if it is mixed (gray). Environmental Justice communities ('MA') only evaluated at Block Group scale. Disabled Adults and Housing Burdened only evaluated at Tract and Municipality scales.")
...
Median repair times vary only slightly by utility, but there are significant differences in terms of outliers (see Figure \@ref(fig:boxTimeU)). Liberty Utilities and Berkshire Gas exhibited the highest median repair times, but National Grid showed the highest frequency and magnitudes of outliers. 

```{r boxTimeU, fig.align='center', fig.cap="Boxplot of gas leak repair times by utility in 2019"}
...
People of Color (especially Blacks and Asians), followed by lower income groups, are most exposed to the longest repair times in the state’s three largest utilities – National Grid, Eversource Energy, and Columbia Gas (see Figure \@ref(fig:figDotTimeU)). Interestingly, Whites are also consistently exposed to repair times slightly above the general population in four out of the six utility territories, albeit primarily in the smaller utilities and never as the most exposed group. In utility territories where People of Color are less exposed, it is instead groups with less education (Berkshire Gas), people who are older (Berkshire Gas, Liberty Utilities), and residents of lower income environmental justice communities (Unitil) who are exposed to the longest repair times. 

```{r figDotTimeU, fig.align='center', fig.cap="Relative exposures to population-weighted mean gas leak repair times in 2019 by utility at Census Block Group, Census Tract, and municipality scales"}
# Set up for dot graph to show all three scales by utility
ppLeakDensityJoinedU_bg2 <- ppLeakDensityJoinedU_bg %>% 
  mutate(Group = factor(Group, levels = group_orderBG)) %>% 
...
At the end of calendar year 2019, there were `r format(nrow(unrepaired2019),big.mark = ",")` unrepaired natural gas leaks across the state, representing `r total_Sumdf %>% filter(Class == "Total") %>% select(PctUnrepaired) %>% pull() %>% round(., 1)`% of all leaks reported in that year (see Table (\@ref(tab:tabAge)) and Figure \@ref(fig:histAge)). Most of the unrepaired leaks were Class 3 leaks (`r total_Sumdf %>% filter(Class == "3") %>% select(PctUnrepaired) %>% pull() %>% round(., 1)`%), the least hazardous leak classification with the most lenient repair protocols. Class 2 leaks were `r total_Sumdf %>% filter(Class == "2") %>% select(PctUnrepaired) %>% pull() %>% round(., 1)`% of unrepaired leaks, followed distantly by Class 1 leaks, which constituted only `r total_Sumdf %>% filter(Class == "1") %>% select(PctUnrepaired) %>% pull() %>% round(., 1)`% (`r unrepaired2019 %>% filter(Class == "1") %>% nrow()`) of all unrepaired leaks at the end of 2019. The age of these unrepaired leaks was calculated as the duration of time from the date and time the leak was reported to the end of the calendar year – December 31, 2019. 
...
Geographically, areas with the oldest unrepaired leaks radiate outward from Boston and inner core communities, primarily along a southeast to northwest axis (see Figures \@ref(fig:magBGage) - \@ref(fig:magCOage)). Areas with the oldest unrepaired leaks are concentrated in the eastern half of the state, with some outliers in the southeast and south central reigons. 

```{r magBGage, fig.align='center', fig.cap= "Average unrepaired leak age in 2019 across Massachuseetts by Census Block Group", cache=TRUE}
...
```{r magTRage, fig.align='center', fig.cap= "Average unrepaired leak age in 2019 across Massachuseetts by Census Tract", cache=TRUE}
# create a map of average leak age by block group
...
```{r magCOage, fig.align='center', fig.cap= "Average unrepaired leak age in 2019 across Massachuseetts by municipality (i.e., Census County Subdivision)", cache=TRUE}
...
```{r figDotAgeU, fig.align='center', fig.cap="Relative exposures to population-weighted mean unrepaired gas leak age in 2019 by utility at Census Block Group, Census Tract, and municipality scales"}
# Set up for dot graph to show all three scales by utility
ppLeakDensityJoinedU_bg2 <- ppLeakDensityJoinedU_bg %>% 
  mutate(Group = factor(Group, levels = group_orderBG)) %>% 
...
But residential geography and infrastructure age alone do not explain the gas leaks inequities. The analysis also shows that unrepaired gas leaks are left unrepaired longer in the same communities that also experience higher leak densities. By contrast, leaks in communities that are more racially White and older, are repaired more quickly. The difference in leak age occurs primarily amongst Class 3 (i.e., less hazardous) leaks, which account for the bulk of unrepaired leaks. Although recent research has revealed that a small subset (~7%) of these leaks likely account for 50% of total distribution gas leak volume, utilities have significant discretion over these “non-hazardous” leaks. Systematic differences in the duration over which gas leaks remain unrepaired cannot simply be attributed to geographic happenstance, but must instead be related to the decision-making processes of the utilities themselves, as well as the policies (or lack thereof) that enable this unequal treatment. 
...
*Regular and detailed public reporting are essential for assessments of equity and for accountability.* Utilities should be required to collect and report accurate and precise performance metrics, such as the type and location of distribution-level gas leaks, at the highest geographical and temporal resolution to allow for meaningful analyses of equity. Moreover, data reporting and record keeping should be standardized to allow for comparisons and auditing. The analysis presented here shows very clear patterns of inequity, but this analysis would not have been possible except for the fact that Massachusetts had implemented regulations requiring regular reporting and public disclosure. However, even here, data reporting still required significant effort and resources by a volunteer-based, non-governmental organization (i.e., HEET) to make the data truly accessible to the public and to these researchers. 

*Regulators and utilities need to anticipate and mitigate potentially regressive impacts of the energy transition [@Zhou_Noonan_2019].* Massachusetts has set a statewide goal of carbon neutrality by 2050 and the Attorney General has directed natural gas utilities to report how they will adapt to these mandates [@Volcovici2021]. In the meantime, there is an aggressive movement across the state to push households to adopt renewable energy sources, to implement energy-saving practices and technologies, and to electrify their water and space heating appliances. The cumulative impact of these efforts will be an accelerating decline in household energy intensity and reliance on natural gas. The transition toward sustainable energy sources is happening most rapidly amongst wealthier households, especially homeowners [@Drehobl_Ross_2020; @Borenstein_2017; @Kwan_2012]. Without intervention, this energy transition may mean that the costs of maintaining the natural gas distribution system increasingly fall on a smaller pool of ratepayers in less wealthy households and communities [@Castigliego_2020]. Already regressive cost burdens and impacts may be further exacerbated. A sustainable and just energy transition requires active monitoring of current equity conditions and planning to prevent worsening or creating new inequities in the future
