...
unrepaired2019_with_tract <- ma_tracts18 %>% 
  select(GEOID) %>%
  st_join(unrepaired2019final, ., largest = TRUE)
# do same for repaired leaks
repaired2019_with_tract <- ma_tracts18 %>% 
  select(GEOID) %>%
...
ma_tracts18 <- lapply(list(unrepaired2019_by_tract,
  ...
  left_join(ma_tracts18, ., by = "GEOID") %>% 
...
# join total leak counts to the tract demographics
ma_tracts18 <- lapply(list(repaired2019_by_tract,
...
# 
# summary(ma_tracts18$unrepaired2019total)
# 
# tm_shape(ma_tracts18) + tm_fill(col = "unrepaired2019total", style = "fisher")
# 
# ma_tracts18 %>% 
#   ggplot(aes(x = leaks_sqkm)) + geom_histogram()
# 
# summary(ma_tracts18$leaks_sqkm)
# 
# tm_shape(ma_tracts18) + tm_fill(col = "leaks_sqkm", style = "quantile")
# tm_shape(ma_tracts18) + tm_fill(col = "leaks_sqkm", style = "fisher") + 
...
# 
# summary(ma_tracts18$unrepaired2019totalC1)
# 
# ma_tracts18 %>% 
#   ggplot(aes(x = unrepaired2019totalC2)) + geom_histogram()
# 
# summary(ma_tracts18$unrepaired2019totalC2)
# 
# ma_tracts18 %>% 
#   ggplot(aes(x = unrepaired2019totalC3)) + geom_histogram()
# 
# summary(ma_tracts18$unrepaired2019totalC3)
...
# library(weights)
weights::wtd.t.test(x = ma_tracts18$leaks_sqkm, y = ma_tracts18$leaks_sqkm, 
                    weight = ma_tracts18$nhwhitepop_E, 
                    weighty = ma_tracts18$nhblackpop_E, 
                    bootse = TRUE, bootp = TRUE)

weights::wtd.t.test(x = ma_tracts18$PctRepaired19, 
                    y = ma_tracts18$PctRepaired19, 
                    weight = ma_tracts18$nhwhitepop_E, 
                    weighty = ma_tracts18$eng_limitE, 
                    bootse = TRUE, bootp = TRUE)
