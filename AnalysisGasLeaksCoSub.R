...
ng_service_areas <- st_read(dsn = "Data/townssurvey_shp",
  ...
  st_transform(., crs = st_crs(ma_cosub18)) %>% 
  st_make_valid()

# assign natural gas territories to municipalities and keep only those municipalities in utility territories for which we have leaks data
ma_cosub18 <- ng_service_areas %>% 
  select(-TOWN) %>% 
  st_join(ma_cosub18, ., largest = TRUE) %>% 
...
# assign municipality data to each unrepaired leak it intersects. 
# join municipality info to each leak that falls within it
unrepaired2019_with_municipality <- ma_cosub18 %>% 
  select(GEOID) %>%
  st_join(unrepaired2019final, ., largest = TRUE)
# do same for repaired leaks
repaired2019_with_municipality <- ma_cosub18 %>% 
  select(GEOID) %>%
  st_join(repaired2019final, ., largest = TRUE)

# break them out by leak class
unrepaired2019_with_municipalityC1 <- unrepaired2019_with_municipality %>% 
  filter(Class == "1")

unrepaired2019_with_municipalityC2 <- unrepaired2019_with_municipality %>% 
  filter(Class == "2")

unrepaired2019_with_municipalityC3 <- unrepaired2019_with_municipality %>% 
  filter(Class == "3")
# do same for repaired
repaired2019_with_municipalityC1 <- repaired2019_with_municipality %>% 
  filter(Class == "1")

repaired2019_with_municipalityC2 <- repaired2019_with_municipality %>% 
  filter(Class == "2")

repaired2019_with_municipalityC3 <- repaired2019_with_municipality %>% 
  filter(Class == "3")

# group by GEOID to get a simple count of all leaks within each municipality, as well as avg leak age in days
unrepaired2019_by_municipality <- unrepaired2019_with_municipality %>% 
  group_by(GEOID) %>% 
  summarize(unrepaired2019total = n(),
            LeakAgeDaysAvg = mean(LeakAgeDays, na.rm = TRUE),
            LeakAgeDaysMed = median(LeakAgeDays, na.rm = TRUE))

unrepaired2019_by_municipalityC1 <- unrepaired2019_with_municipalityC1 %>% 
  group_by(GEOID) %>% 
...
unrepaired2019_by_municipalityC2 <- unrepaired2019_with_municipalityC2 %>% 
  group_by(GEOID) %>% 
...
unrepaired2019_by_municipalityC3 <- unrepaired2019_with_municipalityC3 %>% 
  group_by(GEOID) %>% 
...
repaired2019_by_municipality <- repaired2019_with_municipality %>% 
  group_by(GEOID) %>% 
  summarize(repaired2019total = n(), 
            DaysToRepairAvg = mean(DaysToRepair, na.rm = TRUE),
            DaysToRepairMed = median(DaysToRepair, na.rm = TRUE))

repaired2019_by_municipalityC1 <- repaired2019_with_municipalityC1 %>% 
  group_by(GEOID) %>% 
...
repaired2019_by_municipalityC2 <- repaired2019_with_municipalityC2 %>% 
  group_by(GEOID) %>% 
...
repaired2019_by_municipalityC3 <- repaired2019_with_municipalityC3 %>% 
  group_by(GEOID) %>% 
...
# group by two columns to get the count of leaks by unique combinations of Utility and GEOID so that we know which utility is responsible for which leaks in each municipality.
unrepaired2019_by_utility <- unrepaired2019_with_municipality %>% 
  as.data.frame() %>% 
...
unrepaired2019_by_utilityC1 <- unrepaired2019_with_municipalityC1 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
unrepaired2019_by_utilityC2 <- unrepaired2019_with_municipalityC2 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
unrepaired2019_by_utilityC3 <- unrepaired2019_with_municipalityC3 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
repaired2019_by_utility <- repaired2019_with_municipality %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
repaired2019_by_utilityC1 <- repaired2019_with_municipalityC1 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
repaired2019_by_utilityC2 <- repaired2019_with_municipalityC2 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
repaired2019_by_utilityC3 <- repaired2019_with_municipalityC3 %>% 
  as.data.frame() %>% 
  group_by(Utility, GEOID) %>% 
...
# join total leak counts to the municipality demographics
ma_cosub18 <- lapply(list(unrepaired2019_by_municipality,
                           unrepaired2019_by_municipalityC1,
                           unrepaired2019_by_municipalityC2,
                           unrepaired2019_by_municipalityC3), function(x){
                             as.data.frame(x) %>% 
                               select(-geometry)}) %>% 
  reduce(full_join, by = "GEOID") %>% 
  # distinct(GEOID, .keep_all = TRUE) %>% # get rid of duplicate rows
  left_join(ma_cosub18, ., by = "GEOID") %>% 
...
# join leak counts by utility to the municipality demographics
ma_cosub18 <- list(ma_cosub18, 
...
# join total leak counts to the municipality demographics
ma_cosub18 <- lapply(list(repaired2019_by_municipality,
                           repaired2019_by_municipalityC1,
                           repaired2019_by_municipalityC2,
                           repaired2019_by_municipalityC3), function(x){
                             as.data.frame(x) %>% 
                               select(-geometry)}) %>% 
  reduce(full_join, by = "GEOID") %>% 
  left_join(ma_cosub18, ., by = "GEOID") %>% 
...
# # Look at distributions of leak counts by municipality
# ma_cosub18 %>% 
#   ggplot(aes(x = unrepaired2019total)) + geom_histogram()
# 
# summary(ma_cosub18$unrepaired2019total)
# 
# tm_shape(ma_cosub18) + tm_fill(col = "unrepaired2019total", style = "fisher")
# 
# ma_cosub18 %>% 
#   ggplot(aes(x = leaks_sqkm)) + geom_histogram()
# 
# summary(ma_cosub18$leaks_sqkm)
# 
# tm_shape(ma_cosub18) + tm_fill(col = "leaks_sqkm", style = "quantile")
# tm_shape(ma_cosub18) + tm_fill(col = "leaks_sqkm", style = "fisher") + 
...
# 
# summary(ma_cosub18$unrepaired2019totalC1)
# 
# ma_cosub18 %>% 
#   ggplot(aes(x = unrepaired2019totalC2)) + geom_histogram()
# 
# summary(ma_cosub18$unrepaired2019totalC2)
# 
# ma_cosub18 %>% 
#   ggplot(aes(x = unrepaired2019totalC3)) + geom_histogram()
# 
# summary(ma_cosub18$unrepaired2019totalC3)
...
ggsave("Images/LeaksPP_municipality_moe.png")
...
ggsave("Images/LeaksPP_HU_municipality_moe.png")
...
ggsave("Images/LeaksPPall_municipality_moe.png")
...
ggsave("Images/LeaksPPall_HU_municipality_moe.png")
...
ggsave("Images/LeaksPPrepaired_municipality_moe.png")
...
ggsave("Images/LeaksPPrepaired_HU_municipality_moe.png")
...
ggsave("Images/LeaksPPage_municipality_moe.png")
...
ggsave("Images/LeaksPPtimeRep_municipality_moe.png")
...
ggsave("Images/LeaksPPpctFixed_municipality_moe.png")
...
ggsave("Images/LeaksPPbyClass_municipality.png")
...
ggsave("Images/LeaksPPbyClass_HU_municipality.png")
...
ggsave("Images/LeaksPPbyClassAll_municipality.png")
...
ggsave("Images/LeaksPPbyClassAll_HU_municipality.png")
...
ggsave("Images/LeaksPPbyClassREP_municipality.png")
...
ggsave("Images/LeaksPPbyClassREP_HU_municipality.png")
...
ggsave("Images/LeaksPPbyClassAge_municipality.png")
...
ggsave("Images/LeaksPPbyClassTime_municipality.png")
...
ggsave("Images/LeaksPPbyClassPctFix_municipality.png")
...
ggsave("Images/LeaksPPbyUtility_municipality.png")
...
ggsave("Images/LeaksPPbyUtility_HU_municipality.png")
...
ggsave("Images/LeaksPPbyUtilityREP_municipality.png")
...
ggsave("Images/LeaksPPbyUtilityREP_HU_municipality.png")
...
ggsave("Images/LeaksPPbyUtilityALL_municipality.png")
...
ggsave("Images/LeaksPPbyUtilityALL_HU_municipality.png")
...
ggsave("Images/LeaksPPbyUtilityPctFix_municipality.png")
...
ggsave("Images/LeaksPPbyUtilityAge_municipality.png")
...
ggsave("Images/LeaksPPbyUtilityTime_municipality.png")


# conduct a weighted t-test to determine if differences are statistically significant
# library(weights)
weights::wtd.t.test(x = ma_cosub18$leaks_sqkm, y = ma_cosub18$leaks_sqkm, 
                    weight = ma_cosub18$nhwhitepop_E, 
                    weighty = ma_cosub18$nhblackpop_E, 
                    bootse = TRUE, bootp = TRUE)

weights::wtd.t.test(x = ma_cosub18$PctRepaired19, 
                    y = ma_cosub18$PctRepaired19, 
                    weight = ma_cosub18$nhwhitepop_E, 
                    weighty = ma_cosub18$eng_limitE, 
                    bootse = TRUE, bootp = TRUE)
