...
library(spatstat) # calculate weighted.medians
...
saveRDS(ma_blkgrps, file = "Data/ma_blkgrpsMED.Rds")
...
ppLeakDensity <-  ma_blkgrps %>% 
  ...
  group_by(Group) %>% 
  summarize(wLeaksPerSqKm = weighted.median(x = leaks_sqkm, w = Pop),
            wLeaksPerSqKmC1 = weighted.median(x = leaks_sqkmC1, w = Pop),
            wLeaksPerSqKmC2 = weighted.median(x = leaks_sqkmC2, w = Pop),
            wLeaksPerSqKmC3 = weighted.median(x = leaks_sqkmC3, w = Pop),
            wLeaksPerSqKmALL = weighted.median(x = AllLeaks2019_sqkm, 
                                             w = Pop, na.rm = T),
            wLeaksPerSqKmALLC1 = weighted.median(x = AllLeaks2019C1_sqkm, 
                                             w = Pop),
            wLeaksPerSqKmALLC2 = weighted.median(x = AllLeaks2019C2_sqkm, 
                                            w = Pop),
            wLeaksPerSqKmALLC3 = weighted.median(x = AllLeaks2019C3_sqkm, 
                                            w = Pop),
            wLeakAgeDaysAvg = weighted.median(x = LeakAgeDaysAvg,
                                            w = Pop, na.rm = T),
            wLeakAgeDaysAvgC1 = weighted.median(x = LeakAgeDaysAvgC1,
                                            w = Pop, na.rm = T),
            wLeakAgeDaysAvgC2 = weighted.median(x = LeakAgeDaysAvgC2,
                                            w = Pop, na.rm = T),
            wLeakAgeDaysAvgC3 = weighted.median(x = LeakAgeDaysAvgC3,
                                            w = Pop, na.rm = T),
            wLeaksPerSqKmREP = weighted.median(x = REPleaks_sqkm, 
                                             w = Pop),
            wLeaksPerSqKmREPC1 = weighted.median(x = REPleaks_sqkmC1, 
                                               w = Pop),
            wLeaksPerSqKmREPC2 = weighted.median(x = REPleaks_sqkmC2, 
                                               w = Pop),
            wLeaksPerSqKmREPC3 = weighted.median(x = REPleaks_sqkmC3, 
                                               w = Pop),
            wDaysToRepairAvg = weighted.median(x = DaysToRepairAvg, 
                                               w = Pop, na.rm = T),
            wDaysToRepairAvgC1 = weighted.median(x = DaysToRepairAvgC1,
                                               w = Pop, na.rm = T),
            wDaysToRepairAvgC2 = weighted.median(x = DaysToRepairAvgC2,
                                               w = Pop, na.rm = T),
            wDaysToRepairAvgC3 = weighted.median(x = DaysToRepairAvgC3, 
                                               w = Pop, na.rm = T),
            wPctRepaired19 = weighted.median(x = PctRepaired19, w = Pop, 
                                           na.rm = T),
            wPctRepaired19C1 = weighted.median(x = PctRepaired19C1, w = Pop, 
                                           na.rm = T),
            wPctRepaired19C2 = weighted.median(x = PctRepaired19C2, w = Pop, 
                                           na.rm = T),
            wPctRepaired19C3 = weighted.median(x = PctRepaired19C3, w = Pop, 
                                           na.rm = T),
            wLeaksPerHU = weighted.median(x = leaks_hu, w = Pop, 
                                        na.rm = T),
            wLeaksPerHUC1 = weighted.median(x = leaks_huC1, w = Pop, 
                                        na.rm = T),
            wLeaksPerHUC2 = weighted.median(x = leaks_huC2, w = Pop, 
                                          na.rm = T),
            wLeaksPerHUC3 = weighted.median(x = leaks_huC3, w = Pop, 
                                          na.rm = T),
            wREPLeaksPerHU = weighted.median(x = REPleaks_hu, w = Pop, 
                                        na.rm = T),
            wREPLeaksPerHUC1 = weighted.median(x = REPleaks_huC1, w = Pop, 
                                          na.rm = T),
            wREPLeaksPerHUC2 = weighted.median(x = REPleaks_huC2, w = Pop, 
                                          na.rm = T),
            wREPLeaksPerHUC3 = weighted.median(x = REPleaks_huC3, w = Pop, 
                                          na.rm = T),
            wALLLeaksPerHU = weighted.median(x = ALLleaks_hu, w = Pop, 
                                           na.rm = T),
            wALLLeaksPerHUC1 = weighted.median(x = ALLleaks_huC1, w = Pop, 
                                             na.rm = T),
            wALLLeaksPerHUC2 = weighted.median(x = ALLleaks_huC2, w = Pop, 
                                             na.rm = T),
            wALLLeaksPerHUC3 = weighted.median(x = ALLleaks_huC3, w = Pop, 
                                             na.rm = T)) %>% 
...
saveRDS(ppLeakDensityJoined, file = "Data/ppLeakDensityMED.Rds")

# create a table of values with CI and RR
ppLeakDensityJoined %>% 
  filter(!Group %in% c("Native American", "Other race", 
                       "Native Pacific Islander", "Two or more races",
                       "MA_MINORITY17", "MA_MINORITY21", "MA_INCOME17",
                       "MA_INCOME21", "MA_ENGLISH")) %>% 
  arrange(desc(wLeaksPerSqKm)) %>% 
  mutate(`Unrepaired Leaks Per SqKm` = if_else(
    wLeaksPerSqKmUC < wLeaksPerSqKmLC,
    paste0(round(wLeaksPerSqKm,1), " (", 
                                   round(wLeaksPerSqKmUC,1),
                                   " - ", 
                                   round(wLeaksPerSqKmLC,1),
                                   ")"),
    paste0(round(wLeaksPerSqKm,1), " (", 
           round(wLeaksPerSqKmLC,1),
           " - ", 
    ...
    RR = case_when(Group == "Limited English HH" | Group == "Total Households" ~ 
                     wLeaksPerSqKm/wLeaksPerSqKm[Group == "Total Households"], 
                   Group== "Renter Occupied HU" | Group == "Total Occupied HU" ~ 
    ...
    RR_UC = case_when(Group == "Limited English HH" | 
                        Group == "Total Households" ~ 
                        wLeaksPerSqKmUC/wLeaksPerSqKmUC[Group == "Total Households"], 
                      Group== "Renter Occupied HU" | 
                        Group == "Total Occupied HU" ~ 
                        wLeaksPerSqKmUC/wLeaksPerSqKmUC[Group == "Total Occupied HU"], 
                      !Group %in% c("Limited English HH", "Renter Occupied HH",
                                    "Total Households", "Total Occupied HU") ~ 
                        wLeaksPerSqKmUC/wLeaksPerSqKmUC[Group == "Total Population"]),
    RR_LC = case_when(Group == "Limited English HH" | 
                        Group == "Total Households" ~ 
                        wLeaksPerSqKmLC/wLeaksPerSqKmLC[Group == "Total Households"], 
                      Group== "Renter Occupied HU" | 
                        Group == "Total Occupied HU" ~ 
                        wLeaksPerSqKmLC/wLeaksPerSqKmLC[Group == "Total Occupied HU"], 
                      !Group %in% c("Limited English HH", "Renter Occupied HH",
                                    "Total Households", "Total Occupied HU") ~ 
                        wLeaksPerSqKmLC/wLeaksPerSqKmLC[Group == "Total Population"]),
    `Unrepaired Risk Ratio` = if_else(
      RR_UC < RR_LC,
      paste0(round(RR,1), " (", 
             round(RR_UC,1),
             " - ", 
    ...
    `Total Leaks Per SqKm` = if_else(
      wLeaksPerSqKmALLUC < wLeaksPerSqKmALLLC,
      paste0(round(wLeaksPerSqKmALL,1), " (", 
             round(wLeaksPerSqKmALLUC,1),
             " - ", 
             round(wLeaksPerSqKmALLLC,1),
             ")"),
      paste0(round(wLeaksPerSqKmALL,1), " (", 
             round(wLeaksPerSqKmALLLC,1),
             " - ", 
    ...
    RRall = case_when(Group == "Limited English HH" | Group == "Total Households" ~ 
    ...
    RRall_UC = case_when(Group == "Limited English HH" | 
                        Group == "Total Households" ~ 
                        wLeaksPerSqKmALLUC/wLeaksPerSqKmALLUC[Group == "Total Households"], 
                      Group== "Renter Occupied HU" | 
                        Group == "Total Occupied HU" ~ 
                        wLeaksPerSqKmALLUC/wLeaksPerSqKmALLUC[Group == "Total Occupied HU"], 
                      !Group %in% c("Limited English HH", "Renter Occupied HH",
                                    "Total Households", "Total Occupied HU") ~ 
                        wLeaksPerSqKmALLUC/wLeaksPerSqKmALLUC[Group == "Total Population"]),
    RRall_LC = case_when(Group == "Limited English HH" | 
                        Group == "Total Households" ~ 
                        wLeaksPerSqKmALLLC/wLeaksPerSqKmALLLC[Group == "Total Households"], 
                      Group== "Renter Occupied HU" | 
                        Group == "Total Occupied HU" ~ 
                        wLeaksPerSqKmALLLC/wLeaksPerSqKmALLLC[Group == "Total Occupied HU"], 
                      !Group %in% c("Limited English HH", "Renter Occupied HH",
                                    "Total Households", "Total Occupied HU") ~ 
                        wLeaksPerSqKmALLLC/wLeaksPerSqKmALLLC[Group == "Total Population"]),
    `Total Risk Ratio` = if_else(
      RRall_UC < RRall_LC,
      paste0(round(RRall,1), " (", 
             round(RRall_UC,1),
             " - ", 
             round(RRall_LC,1),
             ")"),
  ...
  select(Group, `Total Leaks Per SqKm`, `Total Risk Ratio`, )
...
ppLeakDensityJoined %>% 
  ...
  ylab(expression(paste("Population-weighted median leak density (leaks/", 
                        km^2, ")", " by Census Block Group",sep = ""))) +
  theme_minimal() +
  geom_errorbar(aes(ymin = wLeaksPerSqKmUC, ymax = wLeaksPerSqKmLC)) +
  labs(title = "Priority Populations and Unrepaired Gas Leaks in 2019\nacross Massachusetts", caption = "Based on 5-year ACS 2015-19 estimates. Error bars are 95% confidence interval.")

ggsave("Images/LeaksPP_blkgrpMED_moe.png")
...
#   labs(title = "Priority Populations and Unrepaired Gas Leaks\nPer Housing Unit in 2019 across Massachusetts", caption = "Based on 5-year ACS 2015-19 estimates. Error bars are 95% confidence interval.")
# 
...
ppLeakDensityJoined %>% 
  ...
  labs(title = "Priority Populations and All Repaired and Unrepaired Gas Leaks\nin 2019 across Massachusetts", caption = "Based on 5-year ACS 2015-19 estimates. Error bars are 95% confidence interval.")

ggsave("Images/LeaksPPall_blkgrpMED_moe.png")
...
ggsave("Images/LeaksPPrepaired_blkgrpMED_moe.png")
...
ppLeakDensityJoined %>% 
  ...
  scale_y_continuous(labels = function(x) format(x, big.mark = ",",
                                                 scientific = FALSE)) +
  ylab("Population-weighted median age (days) of unrepaired\nleaks by Census Block Group") +
  theme_minimal() +
  geom_errorbar(aes(ymin = wLeakAgeDaysAvgUC, ymax = wLeakAgeDaysAvgLC)) +
  ggtitle("Priority Populations and Median Age of Unrepaired\nLeaks in 2019 across Massachusetts")

ggsave("Images/LeaksPPage_blkgrpMED_moe.png")
...
ppLeakDensityJoined %>% 
  ...
  geom_col() + coord_flip() + xlab("") + 
  ylab("Population-weighted median leak repair time (days) by Census Block Group") +
  theme_minimal() +
  geom_errorbar(aes(ymin = wDaysToRepairAvgUC, ymax = wDaysToRepairAvgLC)) +
  ggtitle("Priority Populations and Median Leak Repair Times\nin 2019 across Massachusetts")

ggsave("Images/LeaksPPtimeRep_blkgrpMED_moe.png")
...
#   geom_col() + coord_flip() + xlab("") + 
#   ylab("Population-weighted Median Percentage of Leaks Repaired in 2019\nby Census Block Group") +
#   theme_minimal() +
...
# 
# ggsave("Images/LeaksPPpctFixed_blkgrpMED_moe.png")
...
#   labs(x = NULL, 
#        y = expression(paste("Population-weighted median leak density (leaks/", 
#                             km^2, ")", " by Census Block Group",sep = "")),
...
ggsave("Images/LeaksPPbyClassAll_blkgrpMED.png")
...
ppLeakDensity %>% 
  ...
  facet_wrap(~ leakClass, scales = "free") +
  labs(x = NULL, 
       y = "Population-weighted median age (days) of unrepaired leaks by Census Block Group",
       title = "Priority Populations and Median Age of Unrepaired Leaks in 2019 across Massachusetts")

ggsave("Images/LeaksPPbyClassAge_blkgrpMED.png")
...
ppLeakDensity %>% 
  ...
  facet_wrap(~ leakClass, scales = "free") +
  labs(x = NULL, 
       y = "Population-weighted median leak repair time (days) by Census Block Group",
       title = "Priority Populations and Median Leak Repair Time in 2019 across Massachusetts")

ggsave("Images/LeaksPPbyClassTime_blkgrpMED.png")
...
ppLeakDensityNG <- ma_blkgrps %>% 
  ...
  pivot_longer(., cols = totalpop_E:renter_occ_unitsE, names_to = "Group", 
               values_to = "Pop", values_drop_na = TRUE) %>% 
  group_by(Group) %>% 
  summarize(wLeaksPerSqKmNG = weighted.median(x = leaks_sqkmNG, w = Pop, 
                                            na.rm = TRUE),
            wLeaksPerSqKmREPNG = weighted.median(x = REPleaks_sqkmNG, 
                                             w = Pop, na.rm = TRUE),
            wLeaksPerSqKmALLNG = weighted.median(x = AllLeaks2019_sqkmNG, 
                                             w = Pop, na.rm = TRUE),
            wLeakAgeDaysAvgNG = weighted.median(x = LeakAgeDaysAvgNG,
                                              w = Pop, na.rm = TRUE),
            wLeaksPerHUNG = weighted.median(x = leaks_huNG, w = Pop, 
                                        na.rm = T),
            wREPLeaksPerHUNG = weighted.median(x = REPleaks_huNG, 
                                                     w = Pop, na.rm = T),
            wALLLeaksPerHUNG = weighted.median(x = AllLeaks2019_huNG, 
                                                     w = Pop, na.rm = T),
            wPctRepaired19NG = weighted.median(x = PctRepaired19NG, 
                                                     w = Pop, na.rm = T),
            wDaysToRepairAvgNG = weighted.median(x = DaysToRepairAvgNG, 
                                             w = Pop, na.rm = T)) %>% 
  mutate(Group = recode(Group, "hisppop_E" = "Hispanic", 
                        "minority_E" = "People of Color",
                        "nh2morepop_E" = "Two or more races",
...
ppLeakDensityEV <- ma_blkgrps %>% 
  ...
  pivot_longer(., cols = totalpop_E:renter_occ_unitsE, names_to = "Group", 
               values_to = "Pop", values_drop_na = TRUE) %>% 
  group_by(Group) %>% 
  summarize(wLeaksPerSqKmEV = weighted.median(x = leaks_sqkmEV, w = Pop, 
                                            na.rm = TRUE),
            wLeaksPerSqKmREPEV = weighted.median(x = REPleaks_sqkmEV, 
                                               w = Pop, na.rm = TRUE),
            wLeaksPerSqKmALLEV = weighted.median(x = AllLeaks2019_sqkmEV, 
                                               w = Pop, na.rm = TRUE),
            wLeakAgeDaysAvgEV = weighted.median(x = LeakAgeDaysAvgEV,
                                              w = Pop, na.rm = TRUE),
            wLeaksPerHUEV = weighted.median(x = leaks_huEV, w = Pop, 
                                          na.rm = T),
            wREPLeaksPerHUEV = weighted.median(x = REPleaks_huEV, 
                                             w = Pop, na.rm = T),
            wALLLeaksPerHUEV = weighted.median(x = AllLeaks2019_huEV, 
                                             w = Pop, na.rm = T),
            wPctRepaired19EV = weighted.median(x = PctRepaired19EV, 
                                             w = Pop, na.rm = T),
            wDaysToRepairAvgEV = weighted.median(x = DaysToRepairAvgEV, 
                                               w = Pop, na.rm = T)) %>% 
  mutate(Group = recode(Group, "hisppop_E" = "Hispanic", 
                        "minority_E" = "People of Color",
                        "nh2morepop_E" = "Two or more races",
...
ppLeakDensityCG <- ma_blkgrps %>% 
  ...
  pivot_longer(., cols = totalpop_E:renter_occ_unitsE, names_to = "Group", 
               values_to = "Pop", values_drop_na = TRUE) %>% 
  group_by(Group) %>% 
  summarize(wLeaksPerSqKmCG = weighted.median(x = leaks_sqkmCG, w = Pop, 
                                            na.rm = TRUE),
            wLeaksPerSqKmREPCG = weighted.median(x = REPleaks_sqkmCG, 
                                               w = Pop, na.rm = TRUE),
            wLeaksPerSqKmALLCG = weighted.median(x = AllLeaks2019_sqkmCG, 
                                               w = Pop, na.rm = TRUE),
            wLeakAgeDaysAvgCG = weighted.median(x = LeakAgeDaysAvgCG,
                                              w = Pop, na.rm = TRUE),
            wLeaksPerHUCG = weighted.median(x = leaks_huCG, w = Pop, 
                                          na.rm = T),
            wREPLeaksPerHUCG = weighted.median(x = REPleaks_huCG, 
                                             w = Pop, na.rm = T),
            wALLLeaksPerHUCG = weighted.median(x = AllLeaks2019_huCG, 
                                             w = Pop, na.rm = T),
            wPctRepaired19CG = weighted.median(x = PctRepaired19CG, 
                                             w = Pop, na.rm = T),
            wDaysToRepairAvgCG = weighted.median(x = DaysToRepairAvgCG, 
                                               w = Pop, na.rm = T)) %>% 
  mutate(Group = recode(Group, "hisppop_E" = "Hispanic", 
                        "minority_E" = "People of Color",
                        "nh2morepop_E" = "Two or more races",
...
ppLeakDensityFG <- ma_blkgrps %>% 
  ...
  pivot_longer(., cols = totalpop_E:renter_occ_unitsE, names_to = "Group", 
               values_to = "Pop", values_drop_na = TRUE) %>% 
  group_by(Group) %>% 
  summarize(wLeaksPerSqKmFG = weighted.median(x = leaks_sqkmFG, w = Pop, 
                                            na.rm = TRUE),
            wLeaksPerSqKmREPFG = weighted.median(x = REPleaks_sqkmFG, 
                                               w = Pop, na.rm = TRUE),
            wLeaksPerSqKmALLFG = weighted.median(x = AllLeaks2019_sqkmFG, 
                                               w = Pop, na.rm = TRUE),
            wLeakAgeDaysAvgFG = weighted.median(x = LeakAgeDaysAvgFG,
                                              w = Pop, na.rm = TRUE),
            wLeaksPerHUFG = weighted.median(x = leaks_huFG, w = Pop, 
                                          na.rm = T),
            wREPLeaksPerHUFG = weighted.median(x = REPleaks_huFG, 
                                             w = Pop, na.rm = T),
            wALLLeaksPerHUFG = weighted.median(x = AllLeaks2019_huFG, 
                                             w = Pop, na.rm = T),
            wPctRepaired19FG = weighted.median(x = PctRepaired19FG, 
                                             w = Pop, na.rm = T),
            wDaysToRepairAvgFG = weighted.median(x = DaysToRepairAvgFG, 
                                               w = Pop, na.rm = T)) %>% 
  mutate(Group = recode(Group, "hisppop_E" = "Hispanic", 
                        "minority_E" = "People of Color",
                        "nh2morepop_E" = "Two or more races",
...
ppLeakDensityLU <- ma_blkgrps %>% 
  ...
  pivot_longer(., cols = totalpop_E:renter_occ_unitsE, names_to = "Group", 
               values_to = "Pop", values_drop_na = TRUE) %>% 
  group_by(Group) %>% 
  summarize(wLeaksPerSqKmLU = weighted.median(x = leaks_sqkmLU, w = Pop, 
                                            na.rm = TRUE),
            wLeaksPerSqKmREPLU = weighted.median(x = REPleaks_sqkmLU, 
                                               w = Pop, na.rm = TRUE),
            wLeaksPerSqKmALLLU = weighted.median(x = AllLeaks2019_sqkmLU, 
                                               w = Pop, na.rm = TRUE),
            wLeakAgeDaysAvgLU = weighted.median(x = LeakAgeDaysAvgLU,
                                              w = Pop, na.rm = TRUE),
            wLeaksPerHULU = weighted.median(x = leaks_huLU, w = Pop, 
                                          na.rm = T),
            wREPLeaksPerHULU = weighted.median(x = REPleaks_huLU, 
                                             w = Pop, na.rm = T),
            wALLLeaksPerHULU = weighted.median(x = AllLeaks2019_huLU, 
                                             w = Pop, na.rm = T),
            wPctRepaired19LU = weighted.median(x = PctRepaired19LU, 
                                             w = Pop, na.rm = T),
            wDaysToRepairAvgLU = weighted.median(x = DaysToRepairAvgLU, 
                                               w = Pop, na.rm = T)) %>% 
  mutate(Group = recode(Group, "hisppop_E" = "Hispanic", 
                        "minority_E" = "People of Color",
                        "nh2morepop_E" = "Two or more races",
...
ppLeakDensityBG <- ma_blkgrps %>% 
  ...
  pivot_longer(., cols = totalpop_E:renter_occ_unitsE, names_to = "Group", 
               values_to = "Pop", values_drop_na = TRUE) %>% 
  group_by(Group) %>% 
  summarize(wLeaksPerSqKmBG = weighted.median(x = leaks_sqkmBG, w = Pop, 
                                            na.rm = TRUE),
            wLeaksPerSqKmREPBG = weighted.median(x = REPleaks_sqkmBG, 
                                               w = Pop, na.rm = TRUE),
            wLeaksPerSqKmALLBG = weighted.median(x = AllLeaks2019_sqkmBG, 
                                               w = Pop, na.rm = TRUE),
            wLeakAgeDaysAvgBG = weighted.median(x = LeakAgeDaysAvgBG,
                                              w = Pop, na.rm = TRUE),
            wLeaksPerHUBG = weighted.median(x = leaks_huBG, w = Pop, 
                                          na.rm = T),
            wREPLeaksPerHUBG = weighted.median(x = REPleaks_huBG, 
                                             w = Pop, na.rm = T),
            wALLLeaksPerHUBG = weighted.median(x = AllLeaks2019_huBG, 
                                             w = Pop, na.rm = T),
            wPctRepaired19BG = weighted.median(x = PctRepaired19BG, 
                                             w = Pop, na.rm = T),
            wDaysToRepairAvgBG = weighted.median(x = DaysToRepairAvgBG, 
                                               w = Pop, na.rm = T)) %>% 
  mutate(Group = recode(Group, "hisppop_E" = "Hispanic", 
                        "minority_E" = "People of Color",
                        "nh2morepop_E" = "Two or more races",
