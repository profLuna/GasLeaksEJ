...
### THIS IS A VERSION WITH NO CAP ON LC VALUES, SO POSSIBLY NEGATIVE VALUES
...
ma_blkgrps <- get_acs(geography = "block group", 
  ...
  mutate(totalpopE_UC = totalpopE + totalpopM,
         totalpopE_LC = totalpopE - totalpopM,
...
medhhinclt50 <- B19001 %>%
  ...
  mutate(medhhinclt50_UC = medhhinclt50E + medhhinclt50M,
         medhhinclt50_LC = medhhinclt50E - medhhinclt50M)
...
medhhinclt50_pct <- medhhinclt50 %>%
  ...
  mutate(r2medhhincE = ifelse(householdsE <= 0, 0, medhhinclt50E/householdsE),
    ...
    pct_medhhinclt50E_LC = pct_medhhinclt50E - pct_medhhinclt50M) %>%
...
povknown <- C17002 %>% 
  ...
  transmute(GEOID = GEOID,
            ...
            povknownE_LC = povknownE - povknownM)
...
B03002_minority <- get_acs(geography = "block group", 
  ...
  mutate(minority_UC = minority_E + minority_M,
         minority_LC = minority_E - minority_M)
...
eng_limited_pct <- eng_limited %>% 
  ...
  transmute(eng_hhE = estimate,
            ...
            eng_limit_pctE_LC = eng_limit_pctE - eng_limit_pctM) %>% 
...
lths_pct <- age25up %>% 
  ...
  mutate(r_lthsE = ifelse(
    ...
    pct_lthsE_LC = pct_lthsE - pct_lthsM) %>% 
  ...
  mutate(r_collegeE = ifelse(
    ...
    pct_collegeE_LC = pct_collegeE - pct_collegeM) %>% 
...
age5_64_pct <- allAges %>% 
  ...
  mutate(r_under5E = ifelse(
    ...
    pct_under5E_LC = pct_under5E - pct_under5M) %>% 
  ...
  mutate(r_over64E = ifelse(
    ...
    pct_over64E_LC = pct_over64E - pct_over64M) %>% 
  select(-starts_with("r_"))
# clean up
...
# save output
save(ma_blkgrps, file = "/Data/Demographics0.rds")
# clear environment
# rm(list = ls())
...
house_burdened <- get_acs(geography = "tract", 
  ...
  mutate(house_burdened_UC = house_burdened_E + house_burdened_M,
         house_burdened_LC = house_burdened_E - house_burdened_M)
...
disabledOver18 <- B18101 %>%
  ...
  mutate(disabledOver18E_UC = disabledOver18E + disabledOver18M,
         disabledOver18E_LC = disabledOver18E - disabledOver18M)
# Join the tables and compute derived proportions with MOEs
disabilityOver18_pct <- over18 %>% 
  ...
  mutate(r_disabilityOver18E = ifelse(
    ...
    pct_disabilityOver18E_LC = pct_disabilityOver18E - pct_disabilityOver18M)%>% 
  select(-starts_with("r_"))
# clean up
...
save(ma_blkgrps, ma_tracts, file = "/Data/Demographics0.rds")
# clear environment
# rm(list = ls())
...
# save output
# load("/Data/Demographics.rds")
save(ma_blkgrps, ma_tracts, ma_cosub, file = "C:/Users/Marcos/Documents/Research/GasLeaksEJ/Data/Demographics0.rds")
