...
unrepaired2019_with_municipality <- ma_cosub %>% 
  select(GEOID) %>%
  st_join(unrepaired2019final, ., largest = TRUE)
# do same for repaired leaks
repaired2019_with_municipality <- ma_cosub %>% 
  select(GEOID) %>%
...
ma_cosub <- lapply(list(unrepaired2019_by_municipality,
...
# join total leak counts to the municipality demographics
ma_cosub <- lapply(list(repaired2019_by_municipality,
